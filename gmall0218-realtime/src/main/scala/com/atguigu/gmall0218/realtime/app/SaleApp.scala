        ...
        val saleDetailDstream: DStream[SaleDetail] = fullJoinDStream.mapPartitions { partitionItr =>
            ...
            for ((orderId, (orderInfoOption, orderDetailOption)) <- partitionItr) {
                if (orderInfoOption.isDefined) {
                    ...
                    println(" 主表有数据 ！写入缓存")
                    val orderInfoKey: String = "order_info:" + orderId
                    ...
                    // 3 查询缓存
                    val orderDetailKey: String = "order_detail:" + orderId
                    val orderDetailJson: String = jedis.get(orderDetailKey)
                    val orderDetailSet: util.Set[String] = jedis.smembers(orderDetailJson)
                ...
                } else if (orderDetailOption.isDefined) { //主表没有数据 从表有数据
